...
void EnemyBulletLayer::bossStartShooting() {
	this->schedule(CC_SCHEDULE_SELECTOR(EnemyBulletLayer::addBossBulletSet), UserDefault::getInstance()->getFloatForKey("intervalOfAddEnemyBullet"));
	this->scheduleUpdate();
}

void EnemyBulletLayer::bossStopShooting() {
	this->unschedule(CC_SCHEDULE_SELECTOR(EnemyBulletLayer::addBossBulletSet));
	...
	for (Sprite* bullet : this->allEnemyBullet) {
		if (bullet->getBoundingBox().intersectsRect(myPlane->getBoundingBox())) {
			...
			if(static_cast<PlaneUserData*>(myPlane->getUserData())->isAliveUnderAttack(200) == false){
				...
				scheduleOnce(CC_SCHEDULE_SELECTOR(EnemyBulletLayer::changeSceneCallBack), 1.0f);

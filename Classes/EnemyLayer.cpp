...
void EnemyLayer::startAddEnemy() {
	this->schedule(CC_SCHEDULE_SELECTOR(EnemyLayer::addEnemySprite), UserDefault::getInstance()->getFloatForKey("intervalOfAddEnemy"));
}

void EnemyLayer::stopAddEnemy() {
	this->unschedule(CC_SCHEDULE_SELECTOR(EnemyLayer::addEnemySprite));
}
...
void EnemyLayer::update(float useless) {
	...
	if ((allEnemy.empty() == true) && (this->bossAppeared == true)) {
		...
		scheduleOnce(CC_SCHEDULE_SELECTOR(EnemyLayer::changeSceneCallBack), 1.0f);
	}
